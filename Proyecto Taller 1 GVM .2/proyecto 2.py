...
def ventana():
    ...
    def about():
        ...
        canvasabt = tk.Canvas(aboutgame, width=800, height=600, bg="#1C1A59")
        canvasabt.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

        yoimg = (Image.open('yo.gif'))
        resizedyo = yoimg.resize((200, 267), Image.LANCZOS)
        newyo = ImageTk.PhotoImage(resizedyo)

        yoimg = canvasabt.create_image(10, 10, anchor=tk.NW, image=newyo)

        aboutme = tk.Label(aboutgame,
                           text='ITCR \nTaller de Programacion\nIngenería en Computadores\n2023\nProfesor: Jeff Schmidt Peralta\nCosta Rica\nPython 3.11.2\nEstudiante: Giancarlo Vega Marín',
                           font=('Fixedsys', 16), bg='powder blue', fg='black')
        aboutme.place(x=350, y=50)

        gamemechanics = tk.Label(aboutgame,
                                 text="El juego consiste en matar tantos enemigos alienigenas\n como puedas antes de que el tiempo termine\nUsa las teclas de movimiento para mover tu nave, y utiliza el espacio\n para disparar a tus enemigos\nPosees municion ilimitada, pero tambien tus enemigos. Evitalos",
                                 font=('Fixedsys', 16), bg='powder blue', fg='black')
        gamemechanics.place(x=100, y=300)

        ventanamain.withdraw()

        aboutgame.protocol('WM_DELETE_WINDOW', lambda: destruye_ventana(aboutgame, ventanamain))

        volver = tk.Button(canvasabt, text="Volver", width=10, command=lambda: destruye_ventana(aboutgame, ventanamain))
        volver.place(x=75, y=550)
    ...
    def gamestart():
        ...
        pygame.mixer.init()
        pygame.mixer.music.load("main game.wav")
        pygame.mixer.music.play(-1)
        ...
        def end():  # termina el juego
            if vidas <= 0:
                ...
                gameoverscore = tk.Label(canvasGO, text='Tu puntuacion fue:' + str(puntos), font=('Fixedsys', 32),
                                         bg='#3A4BA1', fg='#B7B1F3')
                gameoverscore.place(x=300, y=450)

                volver = tk.Button(canvasGO, text="Volver", font='Fixedsys', width=10,
                                   command=lambda: destruye_ventana(gameover, ventanamain))
        ...
        def levelpassed():  # el nivel se completo
            if tiempo == 0:
                ...
                gamepassedscore = tk.Label(canvasGG, text='Tu puntuacion fue:' + str(puntos), font=('Fixedsys', 32),
                                           bg='#3A4BA1', fg='#B7B1F3')
                gamepassedscore.place(x=300, y=450)

                volver = tk.Button(canvasGG, text="Volver", width=10,
                                   command=lambda: destruye_ventana(gamepassed, ventanamain))
        ...
        def alienshoot(canvas, alien):  # balas de los aliens
            ...
            current_time = time.monotonic()
            if current_time - lastenemyshottime >= 0.8:  # cooldown
        ...
        # Boton de Regreso
        volver = tk.Button(game, text="Volver", font="Fixedsys", width=10,
                           command=lambda: (destruye_ventana(game, ventanamain)))
    ...
    # --------------------------------------------------------------------------------------------------------------------
    aboutthegame = tk.Button(ventanamain, text="About the game", font=('Fixedsys', 28), bg="#b7b1f3", padx=10, pady=10,
                             command=about)
    aboutthegame.place(x=210, y=150)
    # ---------------------------------------------------------------------------------------------------------------------
    gamebegin = tk.Button(ventanamain, text="Start game", font=('Fixedsys', 28), bg="#b7b1f3", padx=10, pady=10,
                          command=gamestart)
    gamebegin.place(x=210, y=300)
    # ---------------------------------------------------------------------------------------------------------------------
    HIscore = tk.Button(ventanamain, text="High Score", font=('Fixedsys', 28), bg="#b7b1f3", padx=10, pady=10,
                        command=highscore)
